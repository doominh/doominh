...
import { useCreateMenuDetailMutation } from '~/services/menuDetail/menuDetailApi.service';
...
const ModalCreateFood: React.FC<Props> = ({ idMenu, refetchFoods }) => {
  const [formData, setFormData] =
    useState<Omit<MenuDetail, '_id'>>(initialState);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [addFood] = useCreateMenuDetailMutation();
  ...
  const validate = () => {
    ...
    if (!formData.image) {
      newErrors.image = t(
        'adminPage.menuManagement.menuDetail.modal.error.valid_foodImg'
      );
    }
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    await addFood(formDataSend as unknown as Omit<MenuDetail, '_id'>).unwrap();
    handleModalClose();
    (document.getElementById('modal_add_food') as HTMLDialogElement)?.close();
    refetchFoods();
  };

  return (
    <>
      <dialog id="modal_add_food" className="modal">
        <div className="modal-box rounded-lg bg-white p-6 shadow-lg">
          <h3 className="mb-4 text-center text-2xl font-bold">
            {t('adminPage.menuManagement.menuDetail.modal.addFood')}
          </h3>
          <form method="dialog">
            <Button
            ...
            <div className="flex flex-col">
              ...
              <input
                ...
                value={formData.price === 0 ? '' : Number(formData.price)}
                min="0"
                onChange={event =>
                  setFormData(prev => ({
                    ...prev,
            ...
            <div className="flex flex-col">
              ...
              <input
                ...
                id="image"
                name="image"
                className={`file-input w-full p-2 ${errors.image ? 'border-red-500' : ''}`}
                ref={fileInputRef}
                onChange={event => {
